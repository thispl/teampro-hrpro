...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_list=None):
	...
	for emp in employee_map:
		...
		if not filters.summarized_view:
			row += [total_1,total_2,total_3,total_g,total_p, total_l, total_a, total_h, total_um]
...
def get_columns(filters):
	...
	if not filters.summarized_view:
		columns += [_("1st Shift") + ":Float:120",_("2nd Shift") + ":Float:120",_("3rd Shift") + ":Float:120",_("General Shift") + ":Float:120",_("Total Present") + ":Float:120", _("Total Leaves") + ":Float:120",  _("Total Absent") + ":Float:120", _("Total Holidays") + ":Float:120", _("Unmarked Days")+ ":Float:120"]
	return columns, days

def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status,shift,attendance_request,leave_type from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
	...
	for d in attendance_list:
		...
		if d.status in ["Absent","Half Day","Work From Home"]:
			att_map[d.employee][d.day_of_month] = d.status
