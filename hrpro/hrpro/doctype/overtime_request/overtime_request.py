...
class OvertimeRequest(Document):
    def on_submit(self):
        if self.workflow_state == 'Approved':
            ...
            from_date = datetime.date(datetime.strptime(str(self.request_date),'%Y-%m-%d'))
            to_date = datetime.date(datetime.strptime(str(self.request_date),'%Y-%m-%d'))
            ftr = [3600,60,1]
            ...
            frappe.db.commit()
    def validate(self):
        today = frappe.db.sql("select count(*) as count from `tabOvertime Request` where employee = '%s' and request_date = '%s' and name != '%s' and workflow_state != 'Rejected' "%(self.employee,self.request_date,self.name),as_dict=True)
